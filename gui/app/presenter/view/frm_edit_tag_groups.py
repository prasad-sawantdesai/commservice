...
class FrmEditTagGroups(QDialog):

		def __init__(self, id):
				super(FrmEditTagGroups, self).__init__()
				self.id = id
				self.setWindowTitle("Edit tag groups")
				...
				row = obj_db_management.select_taggroup_by_id(self.id)[0]
				self.tag_group_name.setText(row[1])
				self.controller_collection.setCurrentText(row[2])
				self.machine_collection.setCurrentText(row[3])
				self.collection_method.setText(str(row[4]))
				self.collection_type.setText(row[5])
		...
		def store(self):
				try:
						...
						obj_db_management.update_taggroup_by_id(tag_group, self.id)
						QMessageBox.information(self, 'Tag groups', "Tag group updated successfully",
																 QMessageBox.Ok)
						self.close()
