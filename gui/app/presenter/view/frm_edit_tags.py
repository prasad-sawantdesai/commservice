...
class FrmEditTags(QDialog):
		NumGridRows = 3
		NumButtons = 4

		def __init__(self, id):
				super(FrmEditTags, self).__init__()
				self.id = id
				...
				row = obj_db_management.select_tag_by_id(self.id)[0]

				self.tag_name.setText(str(row[1]))
				self.register_types.setCurrentText(str(row[2]))
				self.address.setText(str(row[3]))
				self.scaling.setText(str(row[4]))
				self.data_types.setCurrentText(str(row[5]))
				self.data_size.setText(str(row[6]))
		...
		def store(self):
				try:
						...
						tag = (self.tag_name.text(),
									 ...
									 self.data_size.text())
						obj_db_management.update_tag_by_id(tag, self.id)
						QMessageBox.information(self, 'Tags', "Tag information updated successfully",
																 QMessageBox.Ok)
